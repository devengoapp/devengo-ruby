...
RSpec.describe Devengo::API::AuthService, :integration, type: :api do
  ...
  describe "login" do
    ...
    it "token with expected data", authenticate: false do
      ...
      expect(tokens.token).to be_a Devengo::Resources::Auth::Token
      expect(tokens.token.value).to eq "eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoibWFuXzdQMkhhUGFjNms4R1g1T3J6eDA2MEYiLCJ1c2VyX3R5cGUiOiJNYW5hZ2VyIiwiY29tcGFueV9pZCI6bnVsbCwiZXhwIjoiMTY3NjA3NDkyMyJ9.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" # rubocop:disable Layout/LineLength
      expect(tokens.token).to be_expired
      expect(tokens.token.expires_at).to eq "2023-02-11T00:22:03Z"
      expect(tokens.refresh_token).to be_a Devengo::Resources::Auth::Token
      expect(tokens.refresh_token.value).to eq "eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoibWFuXzdQMkhhUGFjNms4R1g1T3J6eDA2MEYiLCJ1c2VyX3R5cGUiOiJFbXBsb3llZSIsInJlZnJlc2hfdG9rZW5faWQiOiJyZWZfM0J0WnRNcnJQYVdIZmViT2RzZGwxTyIsImV4cCI6IjE2NzYwNzQ5MjMifQ.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" # rubocop:disable Layout/LineLength
